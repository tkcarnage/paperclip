...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // ISSUE_IDENTIFIER env is a UUID when set from heartbeat wake context; clear it so
  // we fall through to the wake-payload / API lookup below.
  if (UUID_RE.test(workspaceIssueIdentifier)) workspaceIssueIdentifier = "";
  if (!workspaceIssueIdentifier && jinriCfg) {
    // First: parse from wake payload (no network call, always present for issue_assigned wakes)
    ...
    // Second: API lookup using the raw UUID
  ...
  if (jinriCfg && workspaceIssueIdentifier) {
    try {
      ...
      env.ISSUE_IDENTIFIER = workspaceIssueIdentifier;  // overwrite UUID with formatted id
